...
static func get_filesystem_tree(plugin:EditorPlugin)->Tree:
    ...
    return find_node_by_class_path(dock) as Tree
...
static func find_node_by_class_path(node:Node)->Node:
    var res:Node

    var first = 'VBoxContainer'
    var second = 'SplitContainer'
    var third = 'MarginContainer'
    var fourth = 'Tree'
    ...
    for c in node.get_children():
        ...
        for n in c.get_children():
            if n.get_class() != second:
                continue
            for m in n.get_children():
                if m.get_class() != third:
                    continue
                for o in m.get_children():
                    if o.get_class() != fourth:
                        continue
                    res = o
                    return res
